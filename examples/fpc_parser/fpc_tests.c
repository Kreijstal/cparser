...
void test_minimal_program_parsing(void) {
    ...
    if (!result.is_success) {
        ...
        TEST_CHECK(ast->typ == T_PROGRAM);
        TEST_MSG("Expected root AST node to be T_PROGRAM, but got %d", ast->typ);

        // Check the program name identifier
        ast_t* ident_node = ast->child;
        TEST_CHECK(ident_node != NULL);
        if (ident_node) {
            TEST_CHECK(ident_node->typ == T_IDENT);
            TEST_MSG("Expected child node to be T_IDENT, but got %d", ident_node->typ);
