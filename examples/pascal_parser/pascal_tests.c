...
typedef void (*parser_init_func)(combinator_t**);
typedef void (*test_validation_func)(ParseResult);
static void run_parser_test(const char* test_str, parser_init_func init_func, test_validation_func validate_func);

void validate_integer_parsing(ParseResult res) {
...
void test_pascal_integer_parsing(void) {
    run_parser_test("123", init_pascal_expression_parser, validate_integer_parsing);
}

void validate_invalid_input(ParseResult res) {
    TEST_ASSERT(!res.is_success);
...
void test_pascal_invalid_input(void) {
    run_parser_test("1 +", init_pascal_expression_parser, validate_invalid_input);
}

void validate_function_call(ParseResult res) {
    TEST_ASSERT(res.is_success);
...
static void run_parser_test(const char* test_str, parser_init_func init_func, test_validation_func validate_func) {
    ...
    init_func(&p);
    input_t* input = new_input();
    input->buffer = strdup(test_str);
    input->length = strlen(test_str);
    ParseResult res = parse(input, p);
...
void validate_successful_parse(ParseResult res) {
    TEST_ASSERT(res.is_success);
}

void test_pascal_enumerated_type_declaration(void) {
    run_parser_test("program Test; type TMyEnum = (Value1, Value2, Value3); begin end.", init_pascal_complete_program_parser, validate_successful_parse);
}

void test_pascal_simple_const_declaration(void) {
    run_parser_test("program Test; const MyConst = 10; begin end.", init_pascal_complete_program_parser, validate_successful_parse);
}

void test_pascal_var_section(void) {
    run_parser_test("program Test; var i: integer; begin end.", init_pascal_complete_program_parser, validate_successful_parse);
}
